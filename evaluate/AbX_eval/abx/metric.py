...
def boltz_cdr_numbering(heavy_str, light_str, heavy_cdr_mask, light_cdr_mask):
    
    assert len(heavy_str) == len(heavy_cdr_mask), print(f'The {heavy_str} not equal {heavy_cdr_mask}')
    assert len(light_str) == len(light_cdr_mask), print(f'The {light_str} not equal {light_cdr_mask}')
    
    def _make_cdr(cdr_mask, chain_type):
        # for cdr_idx in cdr_mask:
        N = len(cdr_mask)
        region_def = np.full((N,), -1)
        last_value = None  # 记录上一个值，初始化为None
        current_idx = -1
        for i in range(len(cdr_mask)):
            if cdr_mask[i] != last_value:  # 如果当前值与上一个值不同，说明进入新区域
                current_idx += 1  # 增加索引
            region_def[i] = current_idx
            last_value = cdr_mask[i]  # 更新上一个值
        if chain_type == 'H':
            return region_def
        elif chain_type == 'L':
            return 7 + region_def
    heavy_cdr = _make_cdr(heavy_cdr_mask, 'H')
    light_cdr = _make_cdr(light_cdr_mask, 'L')
    cdr_def = np.concatenate([heavy_cdr, light_cdr], axis=0)
    return cdr_def, heavy_cdr, light_cdr
...
def modified_make_coords(pdb_file, heavy_chain_id, light_chain_id):
    parser = PDBParser(QUIET=1)
    try:
        model = parser.get_structure('pdb', pdb_file)[0]
    ...
    residues = heavy_residues + light_residues
    residue_single_letter_codes = [seq1(residue.get_resname()) for residue in residues]
    str_seq = ''.join(residue_single_letter_codes)
...
def boltz_make_coords(pdb_file, antibody_id_list):
    parser = PDBParser(QUIET=1)
    model = parser.get_structure('pdb', pdb_file)[0]
    # pid, heavy_chain_id, light_chain_id, antigen_chain_id = pdb_file.split('/')[-1].split('.')[0].split('_')
    heavy_chain_id, light_chain_id = antibody_id_list[0], antibody_id_list[1]
...
def boltz_eval_metric(pred_file, pdb_name, reference_data, args):
    # print(f"pred_file: {pred_file}")
    # if '@' in pred_file.split('/')[-1]:
    #     pdb_name = (pred_file.split('/')[-1]).split('@')[0]
    #     code, heavy_chain_id, light_chain_id, antigen_chain_ids = pdb_name.split('_')
    ...
    #     pdb_name = (pred_file.split('/')[-1]).split('.pdb')[0]
    #     code, heavy_chain_id, light_chain_id, antigen_chain_ids = pdb_name.split('_')
    _, heavy_chain_id, light_chain_id, _ = pdb_name.split('_')
    if args.model == 'boltz':
        order_chain = ['A', 'B']
        if heavy_chain_id != '':
    ...
    # if args.scheme == 'imgt':
    #     cdr_def = cdr_numbering(reference_data['heavy_str'], reference_data['light_str'])

    pred_ab_ca, pred_ab_str_seq, pred_heavy_str_seq, pred_light_str_seq= modified_make_coords(pred_file, heavy_chain_id, light_chain_id)
    if pred_ab_ca is None:
        return None
    try:
        assert (gt_ab_ca.shape[0] == pred_ab_ca.shape[0] and gt_ab_ca.shape[0] == cdr_def.shape[0])
        ...
        # print(f'{pred_file}')
        # print('--------------------------------------')
        # print(f"gt_ab_ca.shape[0]: {gt_ab_ca.shape[0]}")
        # print(f"pred_ab_ca.shape[0]: {pred_ab_ca.shape[0]}")
        # print(f"gt_ab_str_seq: {gt_ab_str_seq}")
        # print(f"pred_ab_str_seq: {pred_ab_str_seq}")
        # print('--------------------------------------')
        # We valid the diffab has this problem.
        # gt_ab_ca = gt_ab_ca[1:]
        # gt_ab_str_seq = gt_ab_str_seq[1:]
        # cdr_def = cdr_def[1:]
        return None
    ab_metrics = calc_ab_metrics(gt_ab_ca, pred_ab_ca, cdr_def, gt_ab_str_seq, pred_ab_str_seq, scheme=args.scheme)
    ab_metrics.update(
        {
            'code': pdb_name,
            'file_path': pred_file
