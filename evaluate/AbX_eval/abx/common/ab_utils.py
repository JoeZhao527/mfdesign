...
from abx.new_utils import deviations_from_coords
...
def calc_ab_metrics(gt_coord, pred_coord, cdr_def, gt_str_seq=None, pred_str_seq=None, fr=True, scheme='chothia'):
    # mask = coord_mask * (cdr_def != -1)
    # gt_coord, pred_coord, cdr_def = gt_coord[mask,:], pred_coord[mask, :], cdr_def[mask]
    
    # we want to align the fr, and then cal the rmsd of cdr.
    
    # print(cdr_def)
    # CDR idx value
    if fr:
        cdr_idx = {1, 3, 5, 8, 10, 12}  # 用集合加速查找
        # if in cdr_idx，is False，else True
        # Here for fr idx.
        mask = np.isin(cdr_def, list(cdr_idx), invert=True)
        # print(mask)
    else:
        mask = None
    
    # print(mask)
    # print(np.array(gt_coord).shape)
    # print(np.array(pred_coord).shape)
    pred_aligned, gt_aligned = deviations_from_coords(
                        np.array(pred_coord), 
                        np.array(gt_coord), 
                        mask,
                        mask,
                        )
    pred_aligned = np.transpose(pred_aligned,[1,0])
    gt_aligned = np.transpose(gt_aligned,[1,0])
    ...
    for k, v in cdr_idx.items():
        ...
        if gt_str_seq is not None:
            ...
            if k == 5 and scheme == 'imgt':
                AAR_1 = np.mean([a==b for a, b in zip(gt_s[4:-2], pred_s[4:-2])])
                ret.update({v + '_Loop_AAR':AAR_1})
            elif k == 5 and scheme == 'chothia':
                AAR_1 = np.mean([a==b for a, b in zip(gt_s[2:-2], pred_s[2:-2])])
                ret.update({v + '_Loop_AAR':AAR_1})
